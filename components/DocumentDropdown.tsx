...
export const DocumentDropdown: React.FC<DocumentDropdownProps> = ({
  ...
  const handleValidateDocument = () => {
    ...
    const clientUrl =
      process.env.EXPO_PUBLIC_CLIENT_URL || "https://eduops.cloud";
    const validationUrl = `${clientUrl}/validate-document`;

    // Open validation page in browser
    Linking.canOpenURL(validationUrl)
      .then((supported) => {
        if (supported) {
  ...
  const dropdownHeight = animation.interpolate({
    inputRange: [0, 1],
    outputRange: [0, 120], // Height for 2 items
  });
    ...
    <View style={styles.container}>
      ...
      <Animated.View style={[styles.dropdown, { height: dropdownHeight }]}>
        <TouchableOpacity
          style={[
            styles.dropdownItem,
            currentRoute === "/document" && styles.activeDropdownItem,
          ]}
          onPress={() => navigateToScreen("/document")}
        >
          <Text
            style={[
              styles.dropdownText,
        ...
        <TouchableOpacity
          ...
          onPress={handleValidateDocument}
        >
          <Text style={styles.dropdownText}>Validate Document</Text>
        </TouchableOpacity>
      </Animated.View>
    </View>
  );
